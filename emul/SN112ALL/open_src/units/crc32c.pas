{
  ...
  1) Create a LongInt Variable For the CRC value;
  2) Initialize this With CRCSeed;
  3) Build the CRC Byte-by-Byte With CRC32();
  4) Finish With CRCend()
...
function scrc32(s:string):longint;
...
function scrc32(s:string):longint;
var i:integer; crc:longint;
begin
crc:=crcSeed;{}
for i:=1 to length(s) do crc:=crc32(byte(s[i]),crc);
crc:=crcEnd(crc);{}
scrc32:=crc;
end;
